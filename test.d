...
void Fl_Button.setonly() {
    ...
    for (int i = g.children();i--; ) {
        Fl_Widget * o = *a++;
        if (o != this && o.type() == FL_RADIO_BUTTON) (cast(Fl_Button*) o).value(0);
    }
}
...
void Fl_Button.draw() {
    ...
    if (compact_ && parent()) {
        ...
        if (!active_r()) divider_color = fl_inactive(divider_color);
        if (x() + w() != px + pw) {
...
int Fl_Button.handle(int event) {
    ...
    switch (event) {
        ...
        case FL_KEYBOARD:
            if (Fl.focus() == this && Fl.event_key() == ' ' && !(Fl.event_state() & (FL_SHIFT | FL_CTRL | FL_ALT | FL_META))) {
